...
int query_in_pgtbl(void *pgtbl, vaddr_t va, paddr_t *pa, pte_t **entry)
{
        ...
        res = get_next_ptp(current_ptp, 0, va, &current_ptp, &pte, false, NULL);
        if (res == -ENOMAPPING) {
                return res;
        }

        // l1
        res = get_next_ptp(current_ptp, 1, va, &current_ptp, &pte, false, NULL);
        if (res == -ENOMAPPING) {
                return res;
        }
        ...
        // l2
        res = get_next_ptp(current_ptp, 2, va, &current_ptp, &pte, false, NULL);
        if (res == -ENOMAPPING) {
                return res;
        }
        else if (res == BLOCK_PTP) {
        ...
        // l3
        res = get_next_ptp(current_ptp, 3, va, &current_ptp, &pte, false, NULL);
        if (res == -ENOMAPPING) {
                return res;
        }
        if (entry != NULL) {
                       ...
                       vmr_prop_t flags, int kind, long *rss)
{
        ...
        while (page_num > 0) {
                ...
                pte_t *pte;
                for (int i = 0; i < 3; ++i) {
                        get_next_ptp(current_ptp, i, va, &current_ptp, &pte, true, rss);
                }
...
int unmap_range_in_pgtbl(void *pgtbl, vaddr_t va, size_t len, long *rss)
{
	...
	u64 page_num = len / PAGE_SIZE + (len % PAGE_SIZE > 0);
        ...
        while (page_num > 0) {
                ...
                res = get_next_ptp(current_ptp, 0, va, &current_ptp, &pte, false, rss);
                if (res == -ENOMAPPING) {
                ...
                res = get_next_ptp(current_ptp, 1, va, &current_ptp, &pte, false, rss);
                if (res == -ENOMAPPING) {
                ...
                res = get_next_ptp(current_ptp, 2, va, &current_ptp, &pte, false, rss);
                if (res == -ENOMAPPING) {
