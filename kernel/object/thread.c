...
void create_root_thread(void)
{
        ...
        for (int i = 0; i < meta.phnum; i++) {
                ...
                /* LAB 3 TODO BEGIN */
                // ret = create_pmo(ROUND_UP(memsz + vaddr, PAGE_SIZE) - ROUND_DOWN(vaddr, PAGE_SIZE), PMO_DATA, root_cap_group, 0, &segment_pmo);
                ret = create_pmo(ROUND_UP(memsz + vaddr, PAGE_SIZE) - ROUND_DOWN(vaddr, PAGE_SIZE), PMO_DATA, root_cap_group, 0, &segment_pmo);
                /* LAB 3 TODO END */
                ...
                memcpy((void *)(phys_to_virt(segment_pmo->start) + (vaddr & OFFSET_MASK)), (void *)((unsigned long)&binary_procmgr_bin_start + ROOT_PHDR_OFF + meta.phnum * ROOT_PHENT_SIZE + offset), filesz);
                /* LAB 3 TODO END */
