...
static void choose_new_current_slab(struct slab_pointer *pool, int order)
{
        ...
        /* BLANK BEGIN */
	if (list_empty(&pool->partial_slab_list)) { // all slabs full
		pool->current_slab = init_slab_cache(order, SIZE_OF_ONE_SLAB); // get one from buddy system
	} else {
		pool->current_slab = list_entry(pool->partial_slab_list.next, struct slab_header, node); // change current slab to the first in the partial slab list
		list_del(&pool->current_slab->node); // remove it from the list
	}
        /* BLANK END */
        /* LAB 2 TODO 2 END */
}
...
static void *alloc_in_slab_impl(int order)
{
         ...
         */
        /* BLANK BEGIN */
	if (current_slab->free_list_head == NULL){ // no free slot in current slab
		choose_new_current_slab(&slab_pool[order], order); // choose a new slab
		current_slab = slab_pool[order].current_slab; // change to the new slab
	}
	free_list = (struct slab_slot_list *)current_slab->free_list_head; // the first free slot in the current slab
	next_slot = free_list->next_free; // the second free slot/NULL
	current_slab->free_list_head = next_slot; // remove the slot from the slab
	current_slab->current_free_cnt -= 1; // update the counts
...
void free_in_slab(void *addr)
{
	...
	slab->free_list_head = (void *)slot; // insert slot to the free_list
	slab->current_free_cnt += 1; // update the number of free slots
        /* BLANK END */
        /* LAB 2 TODO 2 END */

        try_return_slab_to_buddy(slab, order); // if all is freed, place back to buddy
